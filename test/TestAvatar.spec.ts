...
contract("Avatar", async (accounts) => {
  ...
  describe("Avatar.emergencyCall", async () => {
    ...
    it("whitelist emergency1 and set x", async () => {
      ...
      await registry.setWhitelistAvatarCall(mockEmergency1.address, sig, true, {from: a.deployer});
      ...
      // delist
      await registry.setWhitelistAvatarCall(mockEmergency1.address, sig, false, {from: a.deployer});
      await expectRevert(
    ...
    it("whitelist emergency1 and set y", async () => {
      ...
      await registry.setWhitelistAvatarCall(mockEmergency1.address, sig, false, {from: a.deployer});
      await expectRevert(
        avatarInstance.emergencyCall(mockEmergency1.address, setYCallData, {from: a.user1, value: "1"}),
        "emergencyCall: not-listed",
      );

      await registry.setWhitelistAvatarCall(mockEmergency1.address, sig, true, {from: a.deployer});
      await avatarInstance.emergencyCall(mockEmergency1.address, setYCallData, {from: a.user1, value: "1"});
      expect(await mockEmergency1.y()).to.be.bignumber.equal(new BN(555));

      await expectRevert(
        avatarInstance.emergencyCall(mockEmergency2.address, setYCallData, {from: a.user1, value: "1"}),
        "emergencyCall: not-listed",
      );
    });
